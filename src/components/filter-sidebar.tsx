...
// Interface for categories - keeping it simple for frontend-only
interface Category {
  id: string;
  name: string;
  count: number;
...
export default function FilterSidebar({ 
  ...
  selectedExperienceLevels: propExperienceLevels = [], 
  selectedSalaryRanges: propSalaryRanges = [], 
  selectedSources: propSources = [],
  jobStats, 
  sources, 
  onFilterChange 
}: FilterSidebarProps) {
  const [selectedCategories, setSelectedCategories] = useState<string[]>(propCategories);
  const [selectedExperienceLevels, setSelectedExperienceLevels] = useState<string[]>(propExperienceLevels);
  const [selectedSalaryRanges, setSelectedSalaryRanges] = useState<string[]>(propSalaryRanges);
  const [selectedSources, setSelectedSources] = useState<string[]>(propSources);
  const [showAllCategories, setShowAllCategories] = useState(false);

  // Sync state with props when they change
  useEffect(() => {
    setSelectedCategories(propCategories);
  }, [propCategories]);

  useEffect(() => {
    setSelectedExperienceLevels(propExperienceLevels);
  }, [propExperienceLevels]);

  useEffect(() => {
    setSelectedSalaryRanges(propSalaryRanges);
  }, [propSalaryRanges]);

  useEffect(() => {
    setSelectedSources(propSources);
  }, [propSources]);
  ...
  return (
    <aside className="w-full lg:w-64 space-y-4 lg:space-y-6">
      ...
      {sources && sources.length > 0 && (
        <Card className="bg-white shadow-sm border border-slate-200 card-hover animate-scale-in">
          <CardHeader className="pb-3">
            <CardTitle className="font-semibold text-slate-900">Job Sources</CardTitle>
          </CardHeader>
          <CardContent className="pt-0">
            <div className="space-y-2">
              {sources.map((source) => (
                <label key={source.name} className="flex items-center justify-between space-x-2 cursor-pointer filter-item p-2 rounded">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      checked={selectedSources.includes(source.name)}
                      onCheckedChange={(checked) =>
                        handleSourceChange(source.name, checked as boolean)
                      }
                      className="text-[#2563EB] focus:ring-[#2563EB]"
                    />
